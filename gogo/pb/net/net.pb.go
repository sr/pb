...
func (m *EmailAddress) MarshalTo(data []byte) (int, error) {
	...
	var l int
	_ = l
	if len(m.LocalPart) > 0 {
		data[i] = 0xa
		i++
		i = encodeVarintNet(data, i, uint64(len(m.LocalPart)))
		i += copy(data[i:], m.LocalPart)
	}
	if len(m.DomainPart) > 0 {
		data[i] = 0x12
		i++
		i = encodeVarintNet(data, i, uint64(len(m.DomainPart)))
		i += copy(data[i:], m.DomainPart)
	}
	return i, nil
}

func encodeFixed64Net(data []byte, offset int, v uint64) int {
	data[offset] = uint8(v)
...
func encodeFixed32Net(data []byte, offset int, v uint32) int {
	data[offset] = uint8(v)
	data[offset+1] = uint8(v >> 8)
	data[offset+2] = uint8(v >> 16)
	data[offset+3] = uint8(v >> 24)
...
func (m *EmailAddress) Unmarshal(data []byte) error {
	...
	for iNdEx < l {
		...
		case 1:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field LocalPart", wireType)
			}
			...
			m.LocalPart = string(data[iNdEx:postIndex])
			iNdEx = postIndex
		case 2:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field DomainPart", wireType)
			}
			var stringLen uint64
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			m.DomainPart = string(data[iNdEx:postIndex])
			iNdEx = postIndex
		default:
			iNdEx = preIndex
